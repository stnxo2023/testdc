...
class CS16BandwidthTester:
    def __init__(self, server_ip, server_port, num_connections, test_duration,
        ...
        self.storage_dir = storage_dir; self.continuous_mode = continuous_mode
        self.no_server_monitor = no_server_monitor; self.manual_fastdl_url = fastdl_url
        self.file_list_path = file_list_path; self.delete_after_download = delete_after_download
        ...
        self.server_info = None; self.server_rules = None; self.discovered_fastdl_url = None
        self.active_fastdl_url = self.manual_fastdl_url; self.current_map_file_path = None
        self.server_status = "MONITORING_DISABLED" if self.no_server_monitor else "UNKNOWN"
        self.server_last_seen = 0; self.server_info_thread = None; self.server_info_interval = 5; self.server_status_log = []
        self.last_activity = collections.deque(maxlen=LAST_FILES_DISPLAY_COUNT); self.bandwidth_log_points = []
        self.display_thread = None; self.http_session = requests.Session()
        self.files_to_try_http = []; self._load_file_list()
    ...
    def _configure_activity_logger(self):
        ...
        try:
            ...
            file_handler.setFormatter(file_formatter); activity_logger.addHandler(file_handler)
        ...
        except Exception as e: logger.error(f"Failed to configure activity log file handler: {e}"); activity_logger.addHandler(logging.NullHandler())

    def _load_file_list(self):
        self.files_to_try_http = []
        if self.file_list_path and self.active_fastdl_url:
            try:
                ...
                logger.info(f"Loaded {len(self.files_to_try_http)} paths for HTTP from '{self.file_list_path}'")
    ...
    def _log_activity_event(self, worker_id, event_type, status, identifier, bytes_val=0, error_msg="", detail=""):
        ...
        if activity_logger.hasHandlers(): activity_logger.log(log_level, message)
        elif log_level >= logging.WARNING: logger.log(log_level, f"[Activity Log Disabled] {message}")
        ...
        with self.lock:
            if event_type != "UDP Verify":
                 ...
                 elif status != "SUCCESS" and status != "DELETED": ui_entry += f" [{status}]"
                 self.last_activity.append(ui_entry)

    def _simulate_asset_download(self, conn_info):
        conn_id = conn_info['id']; sock = conn_info.get("socket")
        if not sock:
            try:
                if conn_info.get("socket_creation_failed"): return False
                logger.debug(f"Worker {conn_id}: Recreating UDP socket for simulation.")
                sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM); sock.settimeout(2.0); conn_info['socket'] = sock
            except Exception as e:
                logger.error(f"Worker {conn_id}: FAILED recreate socket: {e}. Worker disabled.")
                conn_info["socket_creation_failed"] = True; conn_info['status'] = "error"; self._increment_counter("runtime_connection_issues"); return False
        conn_info["sim_attempts"] = conn_info.get("sim_attempts", 0) + 1; simulated_bytes = 0
        asset_type = random.choice(list(DOWNLOAD_SIZES.keys())); asset_size = DOWNLOAD_SIZES[asset_type]; asset_id = f"{asset_type}_{random.randint(1000,9999)}"
        try:
            conn_info["status"] = "sim_running"; conn_info["last_activity_time"] = time.time()
            base_request = random.choice(CS_FILE_REQUESTS); request = base_request + f" simulate_{asset_id}\x00".encode()
            sent_bytes = sock.sendto(request, (self.server_ip, self.server_port))
            self._increment_counter("udp_requests_sent_sim", 1); conn_info["bytes_sent"] = conn_info.get("bytes_sent", 0) + sent_bytes; conn_info["requests_sent"] = conn_info.get("requests_sent", 0) + 1
            remaining_size = asset_size
            while remaining_size > 0 and self.active and not self._stop_event.is_set():
                bytes_chunk = min(4096, remaining_size, random.randint(1024, 4096))
                self._increment_counter("bytes_received_sim", bytes_chunk); conn_info["bytes_received"] = conn_info.get("bytes_received", 0) + bytes_chunk; simulated_bytes += bytes_chunk
                remaining_size -= bytes_chunk; conn_info["last_activity_time"] = time.time()
                if self.download_delay > 0:
                     chunk_delay = self.download_delay / (asset_size / bytes_chunk) if asset_size > 0 else 0.001
                     if self._stop_event.wait(timeout=max(0.001, chunk_delay)): break
                if self._stop_event.is_set(): break
            if self._stop_event.is_set(): self._log_activity_event(conn_id, "SIM Download", "INTERRUPT", asset_id, simulated_bytes, "Test stopped"); conn_info["status"] = "stopped"; return False
            if remaining_size <= 0: self._increment_counter("simulated_downloads_count"); self._log_activity_event(conn_id, "SIM Download", "SUCCESS", asset_id, simulated_bytes); return True
            else: self._log_activity_event(conn_id, "SIM Download", "INCOMPLETE", asset_id, simulated_bytes); conn_info["status"] = "error"; return False
        except socket.timeout: self._increment_counter("runtime_connection_issues"); conn_info["status"] = "timeout"; self._log_activity_event(conn_id, "SIM Download", "TIMEOUT", asset_id, simulated_bytes, "UDP Send Timeout"); return False
        except OSError as e: self._increment_counter("runtime_connection_issues"); conn_info["status"] = "error"; self._log_activity_event(conn_id, "SIM Download", "OS_ERROR", asset_id, simulated_bytes, str(e)); logger.error(f"Worker {conn_id} Sim OS Error: {e}. Closing socket."); try: sock.close(); conn_info["socket"] = None; except Exception: pass; return False
        except Exception as e: self._increment_counter("runtime_connection_issues"); conn_info["status"] = "error"; self._log_activity_event(conn_id, "SIM Download", "FAILED", asset_id, simulated_bytes, f"{type(e).__name__}: {str(e)}"); logger.error(f"Worker {conn_id} unexpected sim error: {e}", exc_info=self.verbose); return False

    def _perform_http_download(self, conn_info, url, target_filename):
        conn_id = conn_info['id']; timestamp_ms = int(time.time()*1000); safe_base = "".join(c for c in os.path.basename(target_filename) if c.isalnum() or c in ('.', '_', '-')).rstrip()
        local_filename = f"http_dl_{conn_id}_{safe_base}_{timestamp_ms}.tmp"; local_filepath = os.path.join(self.storage_dir, local_filename)
        downloaded_bytes = 0; conn_info["http_attempts"] = conn_info.get("http_attempts", 0) + 1; self._increment_counter("http_download_attempts")
        ...
        try:
            ...
            with self.http_session.get(url, stream=True, timeout=HTTP_TIMEOUT, headers=headers) as response:
                ...
                response.raise_for_status(); os.makedirs(os.path.dirname(local_filepath), exist_ok=True)
                with open(local_filepath, 'wb') as f:
                    for chunk in response.iter_content(chunk_size=8192):
                        if self._stop_event.is_set(): raise InterruptedError("Stop event")
                        if chunk: f.write(chunk); chunk_len = len(chunk); downloaded_bytes += chunk_len; self._increment_counter("bytes_received_http", chunk_len); conn_info["bytes_received"] = conn_info.get("bytes_received", 0) + chunk_len; conn_info["last_activity_time"] = time.time()
            ...
            if content_length and int(content_length) != downloaded_bytes: self._log_activity_event(conn_id, "HTTP Download", "SIZE_WARN", url, downloaded_bytes, f"Size mismatch Exp={content_length}, Got={downloaded_bytes}", detail=f"Path: {local_filepath}")
            conn_info["http_successes"] = conn_info.get("http_successes", 0) + 1; self._increment_counter("http_download_successes"); self._log_activity_event(conn_id, "HTTP Download", "SUCCESS", url, downloaded_bytes, detail=f"Saved to {local_filepath}"); return True
        except requests.exceptions.Timeout as e: self._increment_counter("http_download_failures"); self._increment_counter("runtime_connection_issues"); conn_info["status"] = "timeout"; self._log_activity_event(conn_id, "HTTP Download", "TIMEOUT", url, downloaded_bytes, f"{type(e).__name__}"); return False
        except requests.exceptions.ConnectionError as e: self._increment_counter("http_download_failures"); self._increment_counter("runtime_connection_issues"); conn_info["status"] = "error"; self._log_activity_event(conn_id, "HTTP Download", "CONN_ERR", url, downloaded_bytes, f"{type(e).__name__}"); return False
        except requests.exceptions.HTTPError as e: self._increment_counter("http_download_failures"); conn_info["status"] = "http_error"; self._log_activity_event(conn_id, "HTTP Download", f"HTTP_{e.response.status_code}", url, downloaded_bytes, f"{type(e).__name__}"); return False
        except InterruptedError: conn_info["status"] = "stopped"; self._log_activity_event(conn_id, "HTTP Download", "INTERRUPT", url, downloaded_bytes, "Test stopped"); return False
        except FileNotFoundError as e: self._increment_counter("http_download_failures"); self._increment_counter("runtime_connection_issues"); conn_info["status"] = "error"; self._log_activity_event(conn_id, "HTTP Download", "VERIFY_FAIL", url, downloaded_bytes, str(e)); return False
        except Exception as e: self._increment_counter("http_download_failures"); self._increment_counter("runtime_connection_issues"); conn_info["status"] = "error"; self._log_activity_event(conn_id, "HTTP Download", "FAILED", url, downloaded_bytes, f"{type(e).__name__}: {str(e)}"); logger.error(f"Worker {conn_id}: Unhandled HTTP DL error for {url}: {e}", exc_info=self.verbose); return False
        finally:
            if self.delete_after_download and os.path.exists(local_filepath):
                try: os.remove(local_filepath); self._log_activity_event(conn_id, "HTTP Cleanup", "DELETED", url, detail=f"Path: {local_filepath}")
                except Exception as e: self._log_activity_event(conn_id, "HTTP Cleanup", "DELETE_FAIL", url, error_msg=str(e), detail=f"Path: {local_filepath}")

    def _connection_worker(self, conn_info):
        conn_id = conn_info['id']; mode = "HTTP" if self.active_fastdl_url else "Simulation"
        ...
        try:
            while self.active and not self._stop_event.is_set():
                ...
                if mode == "HTTP":
                    ...
                    if file_rel_path:
                        try: full_url = urljoin(self.active_fastdl_url, file_rel_path.replace('\\', '/')); success = self._perform_http_download(conn_info, full_url, file_rel_path)
                        except Exception as url_err: self._increment_counter("http_download_failures"); self._log_activity_event(conn_id, "HTTP Download", "URL_ERROR", file_rel_path, error_msg=str(url_err)); conn_info["status"] = "error"
                    else: time.sleep(1.5); continue
                else: # Simulation Mode
                    if conn_info.get("socket_creation_failed"): logger.debug(f"Worker {conn_id}: Skipping sim due to socket error."); time.sleep(5); continue
                    success = self._simulate_asset_download(conn_info)
                delay_base = 0.05 if success else 0.2; worker_delay = random.uniform(delay_base, delay_base + 0.1)
                if self._stop_event.wait(timeout=worker_delay): break
        except Exception as e: logger.error(f"Worker {conn_id}: Unhandled loop error: {e}", exc_info=self.verbose); conn_info["status"] = "error"; self._increment_counter("runtime_connection_issues")
        finally: self._decrement_counter("active_workers_count"); conn_info["status"] = "stopped"; sock = conn_info.get("socket");
        if sock: try: sock.close()
        except Exception: pass
    ...
    def _update_server_info(self):
        ...
        while self.active and not self._stop_event.is_set():
            ...
            try:
                ...
                if server_info:
                    info_success = True;
                    with self.lock: self.server_info = server_info; self.server_last_seen = timestamp; ping = server_info.get('ping', -1); current_status = "ISSUES" if ping > HIGH_PING_THRESHOLD else "ONLINE"; self.server_status = current_status; map_name = server_info.get('map');
                ...
                if info_success and (time.time() - last_rules_query_time > query_rules_interval) and not self.manual_fastdl_url:
                    logger.debug("Querying rules for FastDL discovery..."); last_rules_query_time = time.time(); rules = self.server_query.get_rules()
                    with self.lock:
                        if rules:
                             ...
                             if dl_url and (dl_url.startswith('http://') or dl_url.startswith('https://')):
                                  ...
                                  if not self.manual_fastdl_url and self.active_fastdl_url != self.discovered_fastdl_url: logger.info(f"Discovered/Updated FastDL URL: {self.discovered_fastdl_url}"); self.active_fastdl_url = self.discovered_fastdl_url; self._load_file_list()
                             else: self.discovered_fastdl_url = None; self.active_fastdl_url = None
                        else: logger.debug("Rules query failed."); self.server_rules = None; self.discovered_fastdl_url = None; self.active_fastdl_url = None
            except Exception as e: logger.error(f"Server info/rules error: {e}", exc_info=self.verbose);
            with self.lock: current_status = "ERROR"; self.server_info = None; self.server_status = current_status; self.current_map_file_path = None; self.server_rules = None; self.discovered_fastdl_url = None; self.active_fastdl_url = self.manual_fastdl_url
    ...
    def _display_realtime_stats(self):
        ...
        last_bw_log_time = self.start_time; CURSOR_TO_HOME = "\033[H"; CLEAR_LINE = "\033[K"; CLEAR_SCREEN_FROM_CURSOR = "\033[J"; term_width = 80
        try: term_width = os.get_terminal_size().columns
        except OSError: logger.warning("Could not detect terminal width, using 80."); term_width = 80
        while self.active and not self._stop_event.is_set():
            lines_to_print = []
            try:
                with self.lock: elapsed = time.time() - self.start_time if self.start_time > 0 else 0; bytes_http = self.bytes_received_http; http_ok = self.http_download_successes; http_fail = self.http_download_failures; bytes_sim = self.bytes_received_sim; sim_ok = self.simulated_downloads_count; active_workers = self.active_workers_count; runtime_issues = self.runtime_connection_issues; initial_fails = self.initial_connection_failures; current_server_status = self.server_status; current_map_path = self.current_map_file_path; current_ping = self.server_info.get('ping', -1) if self.server_info else -1; current_mode = "HTTP" if self.active_fastdl_url else "Simulation"; display_fastdl_url = self.active_fastdl_url if self.active_fastdl_url else "N/A (Simulation Mode)"; last_activity = list(self.last_activity)
                recv_http_mb = bytes_http / (1024*1024); recv_sim_mb = bytes_sim / (1024*1024); total_recv_mb = recv_http_mb + recv_sim_mb; avg_rate_mbs = total_recv_mb / elapsed if elapsed > 0 else 0
                current_time = time.time()
                if elapsed > 0 and (current_time - last_bw_log_time >= 1.0): with self.lock: self.bandwidth_log_points.append((elapsed, total_recv_mb)); last_bw_log_time = current_time
                status = "Running" if self.active else "Stopped"; test_mode = "Continuous" if self.continuous_mode else f"Timed ({self.test_duration}s)"; header = f"--- CS 1.6 BW Test ({self.server_ip}:{self.server_port} | Mode: {current_mode}) ---".center(term_width); lines_to_print.append(header)
                line_status = f" Status: {status} | Test Mode: {test_mode}"; line_time = f"Time: {elapsed:.1f}s"; lines_to_print.append(f"{line_status:<{max(0, term_width - len(line_time))}}{line_time}")
                line_workers = f" Workers: {active_workers}/{self.num_connections}"; line_issues = f"Issues: Init={initial_fails} Run={runtime_issues}"; lines_to_print.append(f"{line_workers:<{max(0, term_width - len(line_issues))}}{line_issues}")
                lines_to_print.append("-" * term_width); lines_to_print.append("[Server Status (UDP Query)]".center(term_width))
                ...
                else: s_status_str = f" Status: {current_server_status}"; s_ping_str = f"Ping: {current_ping:>3}ms" if current_ping >= 0 else "Ping: N/A"; s_map_str = f"Map Path: {current_map_path if current_map_path else 'UNKNOWN'}"; width_status = len(s_status_str); width_ping = len(s_ping_str); spaces1 = max(1, (term_width - width_status - width_ping) // 2); lines_to_print.append(f"{s_status_str}{' '*spaces1}{s_ping_str}".ljust(term_width)); lines_to_print.append(s_map_str[:term_width].ljust(term_width))
                lines_to_print.append("-" * term_width); lines_to_print.append("[Activity & Bandwidth]".center(term_width)); lines_to_print.append(f" Active FastDL URL: {display_fastdl_url[:term_width-20]}".ljust(term_width))
                ...
                lines_to_print.append(f" Avg Total Rate: {avg_rate_mbs:>6.2f} MB/s".ljust(term_width)); lines_to_print.append("-" * term_width)
                ...
                else: lines_to_print.append("  (No activity yet)".ljust(term_width));
                for _ in range(max(0, LAST_FILES_DISPLAY_COUNT -1)): lines_to_print.append(" ".ljust(term_width))
                lines_to_print.append("-" * term_width); lines_to_print.append("(Press Ctrl+C to stop)".center(term_width))
                output_buffer = CURSOR_TO_HOME;
                for line in lines_to_print: output_buffer += line[:term_width] + CLEAR_LINE + "\n"
                output_buffer += CLEAR_SCREEN_FROM_CURSOR; sys.stdout.write(output_buffer); sys.stdout.flush()
    ...
    def start_test(self):
        ...
        if not self.active_fastdl_url and not self.no_server_monitor:
            ...
            try:
                 ...
                 if rules:
                     ...
                     if dl_url and (dl_url.startswith('http://') or dl_url.startswith('https://')):
                          self.discovered_fastdl_url = dl_url if dl_url.endswith('/') else dl_url + '/'; self.active_fastdl_url = self.discovered_fastdl_url; logger.info(f"Using discovered FastDL URL: {self.active_fastdl_url}")
        ...
        logger.info("="*30 + f" Starting Test (Mode: {mode}) " + "="*30); self.active = True; self._stop_event.clear(); self.start_time = time.time(); self.end_time = 0
        self.bytes_received_http = 0; self.http_download_attempts = 0; self.http_download_successes = 0; self.http_download_failures = 0
        self.bytes_received_sim = 0; self.simulated_downloads_count = 0; self.udp_requests_sent_sim = 0
        self.initial_connection_failures = 0; self.runtime_connection_issues = 0; self.active_workers_count = 0; self.server_status_log = []
        self.last_activity.clear(); self.bandwidth_log_points = []; self.server_status = "MONITORING_DISABLED" if self.no_server_monitor else "UNKNOWN";
        self.server_last_seen = 0; self.server_info = None; self.current_map_file_path = None; self.server_rules = None; self.threads = []
        if not self.no_server_monitor and self.server_query: self.server_info_thread = threading.Thread(target=self._update_server_info, name="ServerMon", daemon=True); self.server_info_thread.start(); self.threads.append(self.server_info_thread)
        self.display_thread = threading.Thread(target=self._display_realtime_stats, name="Display", daemon=True); self.display_thread.start(); self.threads.append(self.display_thread)
        logger.info(f"Starting {self.num_connections} workers (Mode: {mode})..."); self.connections = []; initial_success = 0
        for i in range(self.num_connections):
            if self._stop_event.is_set(): break
            conn_info = {"id": i + 1, "socket": None, "socket_creation_failed": False}; self.connections.append(conn_info)
            worker_thread = threading.Thread(target=self._connection_worker, args=(conn_info,), name=f"Worker-{i+1}", daemon=True); worker_thread.start(); self.threads.append(worker_thread); initial_success += 1
            if self.num_connections > 20 and (i + 1) % 20 == 0: time.sleep(0.05)
        logger.info(f"Launched {initial_success} workers.")
        if initial_success == 0 and self.num_connections > 0: logger.error("FATAL: No workers launched. Stopping."); threading.Thread(target=self.stop_test).start(); return
        try:
            if self.continuous_mode: self._stop_event.wait(); logger.info("Stop event received...")
            else: stopped_early = self._stop_event.wait(timeout=self.test_duration);
    ...
    def stop_test(self):
        ...
        # --- FIX: Corrected indentation for socket closing loop ---
    ...
    def _close_activity_logger(self):
        ...
        if handler:
            try: handler.stream.write(f"--- Activity Log Ended ({datetime.now().isoformat()}) ---\n"); handler.close(); activity_logger.removeHandler(handler); logger.debug("Activity log handler closed.")
            except Exception as e: logger.warning(f"Error closing activity log handler: {e}")

    def _generate_final_report(self, duration):
        print("\n" + "="*30 + " Test Results Summary " + "="*30); duration = max(duration, 0.01)
        with self.lock: bytes_http = self.bytes_received_http; http_ok = self.http_download_successes; http_fail = self.http_download_failures; bytes_sim = self.bytes_received_sim; sim_ok = self.simulated_downloads_count; runtime_issues = self.runtime_connection_issues; initial_fails = self.initial_connection_failures; final_server_status = self.server_status; final_map = self.current_map_file_path or "UNKNOWN"; server_log = self.server_status_log[:]; mode_run = "HTTP" if self.active_fastdl_url else "Simulation"
        recv_http_mb = bytes_http/(1024*1024); recv_sim_mb = bytes_sim/(1024*1024); total_recv_mb = recv_http_mb + recv_sim_mb; avg_rate_mbs = total_recv_mb / duration
        avg_ping = -1; min_ping = -1; max_ping = -1
        if not self.no_server_monitor: pings = [log['ping'] for log in server_log if log.get('ping', -1) >= 0];
        if pings: avg_ping=sum(pings)/len(pings); min_ping=min(pings); max_ping=max(pings)
        print(f"[Test Overview]"); print(f" Target Server:   {self.server_ip}:{self.server_port}"); print(f" Mode Executed:   {mode_run}"); print(f" FastDL URL Used: {self.active_fastdl_url if self.active_fastdl_url else 'N/A'}"); print(f" Duration:        {duration:.2f}s"); print(f" Config Workers:  {self.num_connections}"); print(f" Initial Fails:   {initial_fails}"); print(f" Runtime Issues:  {runtime_issues}")
        if bytes_http > 0 or http_ok > 0 or http_fail > 0: print("\n[HTTP Download Results]"); print(f" Files OK:        {http_ok}"); print(f" Files Failed:    {http_fail} (See {self.activity_log_filename})"); print(f" Received (HTTP): {recv_http_mb:.2f} MB");
        if mode_run == "HTTP": print(f" Files Deleted:   {'Yes' if self.delete_after_download else 'No'}")
        if bytes_sim > 0 or sim_ok > 0: print("\n[Simulation Results]"); print(f" Sim OK:          {sim_ok}"); print(f" Received (Sim):  {recv_sim_mb:.2f} MB"); print(f" Sim Delay Param: {self.download_delay}")
        ...
        else: print(f" Final Status:    {final_server_status}"); print(f" Last Known Map:  {final_map}"); print(f" Avg Ping:        {avg_ping:.1f} ms" if avg_ping != -1 else "N/A"); print(f" Ping Min/Max:    {min_ping} / {max_ping} ms" if min_ping != -1 else "N/A")
        print("="*60)

    def _save_detailed_report_json(self, duration):
         with self.lock: duration = max(0.01, duration); bytes_http = self.bytes_received_http; http_ok = self.http_download_successes; http_fail = self.http_download_failures; bytes_sim = self.bytes_received_sim; sim_ok = self.simulated_downloads_count; total_recv_mb = (bytes_http + bytes_sim) / (1024*1024); avg_rate_mbs = total_recv_mb / duration; server_log = self.server_status_log[:]; mode_run = "HTTP" if self.active_fastdl_url else "Simulation"
         report_data = {
         ...
         ts = int(time.time()); report_filename = f"cs_bw_report_{self.server_ip.replace('.', '_')}_{self.server_port}_{ts}.json"; report_filepath = os.path.join(self.storage_dir, report_filename)
         try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser( description="CS 1.6 Server UDP Query/Simulation Tool (Optional HTTP Downloads)", formatter_class=argparse.ArgumentDefaultsHelpFormatter )
